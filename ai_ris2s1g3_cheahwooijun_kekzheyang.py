...
# Cell 2: Dataset URLs and loader
...
# Cell 3: TF-IDF caching
VECTORS_FILE = "vectors.joblib"
TFIDF_FILE = "tfidf.joblib"

@st.cache_resource
def create_tfidf_model_cached(movies):
    if os.path.exists(VECTORS_FILE) and os.path.exists(TFIDF_FILE):
...
# Cell 4: Collaborative caching
USER_MATRIX_FILE = "user_item_matrix.joblib"
USER_SIM_FILE = "user_sim_df.joblib"

@st.cache_resource
def prepare_collaborative_data_cached(movies, ratings):
    if os.path.exists(USER_MATRIX_FILE) and os.path.exists(USER_SIM_FILE):
        user_item_matrix = joblib.load(USER_MATRIX_FILE)
        user_sim_df = joblib.load(USER_SIM_FILE)
    ...
    joblib.dump(user_item_matrix, USER_MATRIX_FILE)
    joblib.dump(user_sim_df, USER_SIM_FILE)
    return ratings, user_item_matrix, user_sim_df

# Cell 5: Recommendation logic
def content_based_recommend(movie_title, movies, vectors, top_n=10):
    ...
    return "Content-Based", [(t,float(s)) for t,s in zip(recommendations,scores)]

def collaborative_recommend(user_name, user_item_matrix, user_sim_df, top_n=50):
    if user_name not in user_item_matrix.index:
        return {}
    sim_scores = user_sim_df[user_name].drop(user_name, errors='ignore').sort_values(ascending=False)
    ...
    return dict(neighbor_ratings.sort_values(ascending=False).head(top_n))
...
def build_eval_helpers(movies, ratings, vectors):
    ...
    return content_predict, collab_predict, hybrid_predict, ratings_agg, evaluate_model

# Cell 7: Streamlit UI
movies, ratings = load_data()
tfidf, vectors = create_tfidf_model_cached(movies)
ratings, user_item_matrix, user_sim_df = prepare_collaborative_data_cached(movies, ratings)
content_predict, collab_predict, hybrid_predict, ratings_agg, evaluate_model = build_eval_helpers(movies, ratings, vectors)
...
def get_recommendations(movie_title, rec_type, user_choice, num_recs):
    if rec_type=="Content-Based":
        _, recs = content_based_recommend(movie_title, movies, vectors, top_n=num_recs)
    else:
        _, recs = hybrid_recommend(user_choice, movie_title, movies, vectors, user_item_matrix, user_sim_df, top_n=num_recs)
    max_score = max([s for _,s in recs]) if recs else 1.0
    return [(t,(s/max_score)*100) for t,s in recs]

if st.button("Get Recommendations"):
    rec_list = get_recommendations(movie_title, rec_type, user_choice, num_recs)
    ...
    eval_df = pd.DataFrame({
        ...
        "MSE":[evaluate_model(content_predict)[0],
               evaluate_model(collab_predict)[0],
               evaluate_model(hybrid_predict)[0]],
        "RMSE":[evaluate_model(content_predict)[1],
                evaluate_model(collab_predict)[1],
                evaluate_model(hybrid_predict)[1]]
    })
    st.table(eval_df)
