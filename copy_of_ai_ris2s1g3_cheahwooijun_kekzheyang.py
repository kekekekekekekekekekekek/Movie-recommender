...
# Cell 3: Load and preprocess datasets (with full debug)
...
# 🔗 Direct download links (credits must be SMALL version!)
...
credits_path = os.path.join(DATA_PATH, "credits_small.csv")
...
# Load CSVs with try/except
try:
    movies = pd.read_csv(movies_path, low_memory=False)
    st.success(f"Movies loaded ✅ Shape: {movies.shape}")
...
try:
    ...
    st.success(f"Credits loaded ✅ Shape: {credits.shape}")
except Exception as e:
    st.error(f"❌ Failed to load credits: {e}")
    st.stop()

try:
    ratings = pd.read_csv(ratings_path)
    st.success(f"Ratings loaded ✅ Shape: {ratings.shape}")
except Exception as e:
...
try:
    ...
    st.success("IDs converted to int ✅")
except Exception as e:
    st.error(f"❌ ID conversion failed: {e}")
    st.stop()
